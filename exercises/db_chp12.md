...
1. 가장 최근에 생성된 현재 시뭔스 값을 반환하는 의사열 답: CURRVAL
2. 시권스를 변경하는 명령 답: ALTERSEQUENCE
3. 시뭔스를 삭제하는 명렁 답: DROPSEQUENCE
4. 사용자가 수동으로 인텍스를 생성하는 명렁 답: CREATEINDEX
10. 테이블 구조는 남겨두고 저장된 데이터만 삭제하는 명령 답: TRUNCATETABLE
12. 기본 테이블의 구조를 변경하는 명령 답: ALTERTABLE
13. 기본 테이블로부터 만들어지는 가상 테이블인 뷰를 생성하는 영령 답: CREATEVIEW
14. 기본 테이블 구조와 함께 저장된 데이터와 인텍스까지 한꺼번에 삭제하는 영령 답: DROPTABLE
15. DDL 문으로 만들어지지 않고， 질의문 처리 괴정의 중간 결과로 만들어지는 테이블 답: 임시테이블
16. 기존 테이블에 새로운 데이터를 삽입하는 명렁 답: INSERT
19. 시뭔스에 의해 지동으로 생성되는 가상의 열 답: 의사열
20. CREATE VIEW 명령을 이용해서 기몬 테이블로부터 만들어지는 테이블 답: 가상테이블
22. 단 하나의 테이블만 기초로 하여 생성된 뷰  답: 단순뷰
23. 테이블에서 기존 데이터를 갱신히는 명령 답: UPDATE

**세로**

1. 시뭔스를 생성히는 명령 답: CREATESEQUENCE
5. 기존 뷰를 삭제하는 영렁 답: DROPVIEW
6. 기존 테이블에 새로운 데이터를 삽입하는 명령 답: INSERT
7. 테이블에서 기존 데이터를 삭제하는 명령 답: DELETE
8. 기본 테이블을 생성하는 명령 답: CREATETABLE
9. 다음에 사용 가능한 시뭔스 값을 생성하는 의사열 답: NEXTVAL
11. 고정길이 문자열을 기억히는 데이터 타입 답: CHAR
17. 테이블에서 데이터를 검색하는 명렁 답: SELECT
18. 독자적으로 존재하는 테이블 답: 기본테이블
21. 다중 테이블을 기초로 하여 생성된 뷰 답: 복합뷰

## 연습문제

1. DB 구현 단계의 주요 업무가 아닌 것은? 답 2번 틀린 것
   1. DB에 초기 데이터를 삽입한다.
...
ALTER TABLE lab
ADD lab_size INTEGER DEFAULT 50;
...
>
> INSERT INTO lab
VALUES (100, '가상현실', '2공학관', 'B203', 'comp', 50);

INSERT INTO lab
VALUES (110, '인공지능', '2공학관', 'A101', 'comp', 50);
...
> INSERT INTO lab (lab_num, name, building, room_id, dept_id, lab_size)
VALUES (lab_num_seq.NEXTVAL, '네트워크', '2공학관', 'B201', 'comp', 50); --oracle 방식
>
> INSERT INTO lab (lab_num, name, building, room_id, dept_id, lab_size)
VALUES (nextval('lab_num_seq'), '네트워크', '2공학관', 'B201', 'comp', 50); --PosgreSQL 방식
